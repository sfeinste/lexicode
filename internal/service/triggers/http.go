...
//
// POST /triggers/{id}/backtest and GET /projects/{key}/trigger-catalog belong to the trigger
// UI story (S29) and are not served yet.
...
// never collapsed to success/failure (data model §6).
type healthBody struct {
	Counts      map[string]int64 `json:"counts"`
	LastFiredAt *string          `json:"last_fired_at"`
}

// firingBody is one trigger_firings row.
...
func toBodyWithHealth(wh WithHealth) triggerBody {
	...
	h := healthBody{Counts: map[string]int64{}}
